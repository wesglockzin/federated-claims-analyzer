...
from jose.exceptions import JWTError  # used for ADFS access-token introspection in the SSO tester flow
...
# Auth-gate OIDC secrets only required when the gate is on. If any of the
# OIDC vars is missing we silently disable the gate (open posture) — same
# rollback semantics as Okta Change Auditor.
OIDC_ISSUER = os.environ.get("OIDC_ISSUER", "").rstrip("/")
OIDC_CLIENT_ID = os.environ.get("OIDC_CLIENT_ID", "")
OIDC_CLIENT_SECRET = os.environ.get("OIDC_CLIENT_SECRET", "")
APP_BASE_URL = os.environ.get("APP_BASE_URL", "https://localhost:8080").rstrip("/")
OIDC_SCOPES = "openid email profile"
OIDC_GATE_READY = bool(OIDC_ISSUER and OIDC_CLIENT_ID and OIDC_CLIENT_SECRET)

if AUTH_GATE_ENABLED and not OIDC_GATE_READY:
    print("⚠️  WARNING: AUTH_GATE_ENABLED but OIDC_ISSUER / OIDC_CLIENT_ID / OIDC_CLIENT_SECRET incomplete.")
    print("   Auth gate will run in disabled mode (open) until configured.")
...
app.config["SESSION_COOKIE_SAMESITE"] = "None"  # SAML callbacks come back as
# cross-site POSTs from the IdP — Lax would cause the browser to drop the
# session cookie on the POST, breaking SAML state retrieval. None+Secure
# permits cross-site cookies; safe because: (a) the app is HTTPS-only, so the
# cookie is only ever sent over TLS, (b) all sensitive state-changing endpoints
# either signature-validate (SAML callbacks) or state-validate (OIDC callbacks)
# the inbound request, and (c) this is an internal admin tool not exposed to
# arbitrary cross-origin sites.
...
# --- Auth-gate OIDC (PROD Okta via "Admin SSO App" app) ---
# Authlib drives the auth-code+PKCE flow AND validates the id_token.
oauth = OAuth(app)
if OIDC_GATE_READY:
    oauth.register(
        name="okta",
...
PUBLIC_PATHS = {
    # Auth gate's own routes
    "/health", "/login", "/oidc/login", "/oidc/callback", "/logout",
    # SSO TESTER callbacks — must bypass the auth gate. SAML callbacks come in
    # as cross-site POSTs from the IdP; SameSite=Lax drops the session cookie
    # on those, so the auth gate would 302 to /login and the SAML response
    # would be lost. OIDC GET callbacks are unaffected by SameSite=Lax but are
    # listed here for symmetry. Each callback validates internally
    # (python3-saml signature check; OIDC state token).
    "/okta/saml/callback", "/adfs/saml/callback",
    "/okta/oidc/callback", "/adfs/oidc/callback",
}
...
def require_login():
    ...
    if not session.get("user"):
        # Go straight to the IdP — no intermediate "Sign in with Okta" landing
        # page (fleet-uniform direct redirect). /login is kept for logout landing
        # and auth-error display only.
...
# In-tool documentation page (/howto), rendered from HOWTO.md via the shared
# wes_tools_docs module. Inherits the auth gate from the @before_request hook.
register_howto(app, tool_name="Federated Identity & Claims Analyzer")
...
@app.route("/health")
def health():
    return jsonify(status="ok", version=sso_tester_logic.APP_VERSION), 200
