...
OKTA_ENVIRONMENTS = {
    "DEV": {
        "custom": {
            ...
            "oidc_audience": "https://host.example.gov/[redacted-path]",
        },
        "default": {
            "oidc_issuer_url": "https://host.example.gov/[redacted-path]",
            "oidc_client_id": "0oaEXAMPLE00EXAMPLE0",
...
LISTEN_PORT = int(__import__("os").environ.get("PORT", 8080))
VERIFY_SSL = True
...
# Module-level HTTP session — centralizes the User-Agent so IdP-side audit
# logs (notably the Okta Change Auditor's automation classifier) can tell
# this tool apart from generic python-requests automation, and so each
# call site gets connection reuse for free.
_session = make_session("FederatedClaimsAnalyzer", APP_VERSION)
...
def get_jwks_cached(jwks_uri):
    ...
    try:
        ...
        response = _session.get(jwks_uri, verify=VERIFY_SSL, timeout=HTTP_TIMEOUT)
...
def okta_oidc_get_endpoints(issuer_url):
    ...
    try:
        response = _session.get(discovery_url, verify=VERIFY_SSL, timeout=HTTP_TIMEOUT)
        response.raise_for_status()
...
def okta_oidc_exchange_code_for_tokens(code, okta_config, code_verifier):
    ...
    try:
        response = _session.post(OKTA_OIDC_TOKEN_ENDPOINT, data=payload, headers=headers, verify=VERIFY_SSL, timeout=HTTP_TIMEOUT)
        response.raise_for_status()
...
def okta_oidc_get_userinfo_claims(access_token):
    ...
    try:
        response = _session.get(OKTA_OIDC_USERINFO_ENDPOINT, headers=headers, verify=VERIFY_SSL, timeout=HTTP_TIMEOUT)
        response.raise_for_status()
        return response.json()
    except requests.exceptions.RequestException as e:
...
def adfs_oidc_exchange_code_for_tokens(code, adfs_env_key, code_verifier):
    ...
    try:
        response = _session.post(ADFS_OIDC_TOKEN_ENDPOINT, data=payload, headers=headers, verify=VERIFY_SSL, timeout=HTTP_TIMEOUT)
        response.raise_for_status()
        return response.json()
    except requests.exceptions.RequestException as e:
...
def azure_oidc_exchange_code_for_tokens(code, code_verifier):
    ...
    try:
        response = _session.post(AZURE_OIDC_TOKEN_ENDPOINT, data=payload, headers=headers, verify=VERIFY_SSL, timeout=HTTP_TIMEOUT)
        response.raise_for_status()
        return response.json()
    except requests.exceptions.RequestException as e:
...
def adfs_oidc_get_userinfo_claims(access_token):
    ...
    headers = {"Authorization": f"Bearer {access_token}"}
    try:
        response = _session.get(ADFS_OIDC_USERINFO_ENDPOINT, headers=headers, verify=VERIFY_SSL, timeout=HTTP_TIMEOUT)
        response.raise_for_status()
        return response.json()
    except requests.exceptions.RequestException as e:
...
def azure_oidc_get_userinfo_claims(access_token):
    ...
    try:
        ...
        response = _session.get("https://graph.microsoft.com/oidc/userinfo", headers=headers, verify=VERIFY_SSL, timeout=HTTP_TIMEOUT)
        response.raise_for_status()
        return response.json()
    except requests.exceptions.RequestException as e:
...
def oidc_perform_token_refresh(refresh_token, idp_choice, env_key, auth_server_type="custom"):
    ...
    try:
        response = _session.post(token_endpoint, data=payload, headers=headers, verify=VERIFY_SSL, timeout=HTTP_TIMEOUT)
        response.raise_for_status()
...
def build_saml_request_data(req):
    ...
    # This prevents the library from constructing URLs like https://host.example.gov
    if is_https == 'on':
        server_port = '443'
    else:
...
def okta_check_token_lifetimes(environment, auth_server_type="custom"):
    ...
    # For custom: https://host.example.gov/oauth2/{authServerId}
    # For default: https://host.example.gov
    ...
    if "host.example.gov" in issuer_url:
        base_url = "https://host.example.gov"
    elif "host.example.gov" in issuer_url:
        base_url = "https://host.example.gov"
    elif "host.example.gov" in issuer_url:
        base_url = "https://host.example.gov"
    elif "host.example.gov" in issuer_url:
    ...
    try:
        ...
        policies_response = _session.get(policies_url, headers=headers, timeout=10)
        ...
        rules_response = _session.get(rules_url, headers=headers, timeout=10)
